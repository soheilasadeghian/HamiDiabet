...
namespace HamiDiabetWebApi
{
    public class SimpleAuthorizationServerProvider : OAuthAuthorizationServerProvider
    {
        ...
        {  
            ...
            var db = new DataAccessDataContext();
            var ppassword = HamiDiabetWebApi.ClassCollection.Method.convertPersianNumberToEnglishNumber(context.Password);
            var pppassword = HamiDiabetWebApi.ClassCollection.Method.md5(ppassword);
            var user = db.userTbls.SingleOrDefault(c => c.mobile == context.UserName && c.password == pppassword);
            if (user == null)
            {
                context.SetError("invalid_grant", "نام کاربری یا کلمه عبور صحیح نمی باشد");
                context.SetError("نام کاربری یا کلمه عبور صحیح نمی باشد");
                context.Rejected();
                return;
            }
